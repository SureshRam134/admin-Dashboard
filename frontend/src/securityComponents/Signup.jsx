...
function Signup() {
    ...
    const [userSign, setUserSign] = useState({ initial })
    const [userSignErr, setUserSignErr] = useState({ initial })
    const inputHandleFun = e => {
        const { name, value } = e.target;
        setUserSign({ ...userSign, [name]: value })
        setUserSignErr({ ...userSignErr, [name]: "" })
    }
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 md:gap-2  lg:gap-4 items-center bg-primary-light px-[15px] py-[30px]  md:px-[20px] md:py-[20px] rounded-[30px]'>
                <div className='lg:pe-2 w-full md:w-[260px] lg:w-[344px] mx-auto items-center '>
                    ...
                    >
                        ...
                        {userSignErr && <span>{userSignErr.name}</span>}
                        <input
                            className='w-full text-[14.22px] px-[15px] py-[13px] mb-[20px] rounded-lg border-[1px] border-secondary-gray bg-secondary-light text-secondary-light-gray font-lato'
                            id='email'
                            type="text"
                            ...
                            onChange={inputHandleFun} />
                        {userSignErr && <span>{userSignErr.email}</span>}
                        <div className='relative item-center'>
                            <input
                                className='relative w-full text-[14.22px] px-[15px] py-[13px] mb-[20px] rounded-lg border-[1px] border-secondary-gray bg-secondary-light text-secondary-light-gray font-lato'
                                id='password'
                                type="password"
                                ...
                                onChange={inputHandleFun}
                            />
                            {userSignErr && <span>{userSignErr.password}</span>}
                            <span className='absolute right-[11%] top-[24%]'>
                                <svg className='absolute' width="18" height="15" viewBox="0 0 18 15" fill="none" xmlns="http://www.w3.org/2000/svg">
