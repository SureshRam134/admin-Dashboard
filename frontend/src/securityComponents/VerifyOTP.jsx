...
const VerifyuserOtp = () => {
    ...
    const [userOtp, setUserOtp] = useState(initial)
    const [userOtpErr, setUserOtpErr] = useState(initial)

    const inputHandleFun = e => {
        const { name, value } = e.target;
        setUserOtp({ ...userOtp, [name]: value })
        setUserOtpErr({ ...setUserOtpErr, [name]: "" })
    }
    ...
    const handleSubmitFun = async e => {
        ...
        try {
            ...
            const userOtp_res = await axiosURL.post('/user/otp', data)
            alert(userOtp_res.data.message)
            setUserOtp(initial)
            setUserOtpErr(initial)
                    ...
                    >
                        ...
                        <div className='relative flex flex-col'>
                            <input
                                ...
                                value={userOtp.otp}
                                onChange={inputHandleFun} />
                            {userOtpErr && <span className='absolute text-primary-Err text-xs top-[48px]'>{userOtpErr.otp}</span>}
                        </div>
